...
def count_values(a, k):
  #final - passed values and hard test
  counting = [0] * (k+1)
  for i in a:
...
def get_positions(counts):
  # need to finalize if this code will give us full credit
  #and if there is a way to condense the reduce and appends, maybe a for loop ??
  # get pos test passed
  a = []
...
def construct_output(a, positions):
  #was this the one we copied from our last lab?
  # test passed
  list = []
  for i in a:
